  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="shadow-lg">
        <CardHeader>
          <CardTitle className="text-2xl font-headline flex items-center"><Mic className="mr-2 h-6 w-6 text-primary" /> Extempore Practice</CardTitle>
          <CardDescription>
            {currentTopic 
              ? <>Your extempore topic is: <strong className="text-primary">{currentTopic}</strong>. Press "Start Speaking" or "Stop Speaking & Analyze" below.</>
        ...
        <CardContent className="space-y-4">
          {!speechApiSupported && (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                Your browser doesn't support the Web Speech API. Please use a browser like Chrome or Edge for this feature.
              </AlertDescription>
            </Alert>
          )}
          <Button onClick={handleGetTopic} disabled={isLoadingTopic || !speechApiSupported } size="lg">
            {isLoadingTopic ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <BookOpen className="mr-2 h-5 w-5" />}
            Get Extempore Topic
          </Button>
          {currentTopic && (
            <Alert variant="default" className="bg-primary/10 border-primary/30">
              <BookOpen className="h-5 w-5 text-primary" />
              <AlertTitle className="text-primary font-semibold">Your Topic:</AlertTitle>
              <AlertDescription className="text-lg">{currentTopic}</AlertDescription>
            </Alert>
          )}
        </CardContent>
        ...
        {currentTopic && speechApiSupported && ( 
          <CardFooter className="flex-col items-start space-y-4">
             <Button onClick={toggleListening} disabled={isLoadingAnalysis || !currentTopic} size="lg" variant={isListening ? "destructive" : "default"}>
              {isListening ? (
            ...
            <div className="w-full p-3 border rounded-md min-h-[100px] bg-muted/30">
              <p className="text-muted-foreground text-sm mb-1">Your speech transcript (accumulates until analyzed):</p>
              <p>{transcript}</p> 
              {isListening && !transcript && <p className="text-sm text-muted-foreground italic">Waiting for speech...</p>}
              {!isListening && !transcript && !analysisResult && <p className="text-sm text-muted-foreground italic">Click "Start Speaking" to begin once you have a topic. Click "Stop Speaking & Analyze" when done.</p>}
            </div>
          </CardFooter>
        )}
      </Card>
      ...
      {isLoadingAnalysis && (
        <Card className="shadow-md">
          <CardContent className="p-6 text-center">
            ...
            <p className="mt-4 text-muted-foreground">Analyzing your speech...</p>
          </CardContent>
        </Card>
      )}
      ...
      {analysisResult && (
        <div className="space-y-6">
          {analysisResult.overallFeedback && (
            <Card className="shadow-lg">
              <CardHeader>
                <CardTitle className="text-xl font-headline flex items-center"><Sparkles className="mr-2 h-5 w-5 text-accent"/>Overall Feedback</CardTitle>
              </CardHeader>
              <CardContent>
          ...
          <Card className="shadow-lg">
            <CardHeader>
              <CardTitle className="text-xl font-headline">Analyzed & Corrected Transcript</CardTitle>
              <CardDescription>Click on a highlighted section to see the related suggestion. This analysis uses the Genkit grammar check flow.</CardDescription>
            </CardHeader>
            <CardContent>
              <div
                ...
                dangerouslySetInnerHTML={{ __html: analysisResult.highlightedCorrectedText || analysisResult.analyzedText }}
              />
            </CardContent>
          </Card>
          ...
          {analysisResult.suggestions && analysisResult.suggestions.length > 0 && (
            <Card className="shadow-lg">
              <CardHeader>
                <CardTitle className="text-xl font-headline">Detailed Suggestions</CardTitle>
              </CardHeader>
              <CardContent>
                 <ScrollArea className="h-[400px] pr-3">
                    <ul ref={suggestionsContainerRef} className="space-y-3">
                    {analysisResult.suggestions.map((suggestion) => (
                        <li key={suggestion.id} id={`suggestion-${suggestion.id}`}>
                        <Alert variant="default" className={cn("border", getSuggestionTypeColor(suggestion.type))}>
                            {getSuggestionTypeIcon(suggestion.type)}
                            <AlertTitle className="capitalize">{suggestion.type.replace("_", " ")}</AlertTitle>
                            <AlertDescription>
                                <p>{suggestion.text}</p>
                                {suggestion.originalSegment && <p className="mt-1 text-xs italic">Original: "{suggestion.originalSegment}"</p>}
                                {suggestion.suggestedReplacement && <p className="mt-1 text-xs">Suggested: "{suggestion.suggestedReplacement}"</p>}
                            </AlertDescription>
                        </Alert>
                        </li>
                    ))}
           ...
           {!analysisResult.suggestions && analysisResult.overallFeedback && analysisResult.analyzedText === analysisResult.correctedText && (
             <Card className="shadow-md">
                <CardContent className="p-6">
                    <p className="text-muted-foreground">No specific suggestions were generated, likely because the speech was deemed off-topic or no correctable issues were found based on the current analysis scope.</p>
                </CardContent>
             </Card>
           )}
        </div>
