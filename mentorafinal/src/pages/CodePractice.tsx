...
function CodePractice() {
  ...
  type GeminiResponseOrString = GeminiResponse | string;

  const callGeminiAPI = async (
    currentPrompt: string,
    setStateFunction: (data: GeminiResponseOrString) => void,
    ...
    try {
      ...
      setOutput(`${errorMessagePrefix}: ${error instanceof Error ? error.message : 'Check console for details.'}`);
      return false;
    }
  };
      ...
      await callGeminiAPI(
        ...
        (data) => {
          if (typeof data === 'string') {
            try {
              const parsedData = JSON.parse(data.slice(7, -4)) as GeminiResponse;
              console.log("Parsed response:", parsedData);
  
              if (parsedData.error) {
                console.log("Error in API response:", parsedData.error);
                setOutput(parsedData.error);
                setProblem('');
                setHint('');
                setUserInput('');
                setExpectedOutput('');
                ...
                setHint(parsedData.hint || '');
                setUserInput(parsedData.input || '');
                setExpectedOutput(parsedData.output || '');
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          ...
          <p className="text-gray-300 mb-8 max-w-2xl mx-auto">
            Practice coding with AI-generated problems. Get instant feedback and improve your programming skills.
          </p>
        </motion.div>

        <div className="flex flex-col h-[calc(100vh-16rem)] bg-gray-800 rounded-lg shadow-lg border border-gray-700">
          <div className="p-4 bg-gray-800 flex flex-wrap justify-center sm:justify-between items-center border-b border-gray-700">
